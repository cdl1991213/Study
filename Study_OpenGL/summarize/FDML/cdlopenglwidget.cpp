...
QVector3D viewInitPos(0.0, 0.0, 5.0);       //初始光源位置
...
void cdlOpenGLWidget::initializeGL()
{
    ...
    ShaderProgram_ThreeD.setUniformValue("material.diffuse", 4);        //通道4
    ShaderProgram_ThreeD.setUniformValue("material.specular", 5);       //通道5
    ShaderProgram_ThreeD.setUniformValue("material.emission", 6);       //通道6
    //--------------------------------------------------------------------------------------------------
    ...
    ShaderProgram_ThreeD_multiLight.setUniformValue("material.diffuse", 4);         //通道4
    ShaderProgram_ThreeD_multiLight.setUniformValue("material.specular", 5);        //通道5
    //--------------------------------------------------------------------------------------------------
...
//    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);         //纹理过滤（适用于放大）
    ...
    //########绑定VBO#########
    glBindBuffer(GL_ARRAY_BUFFER, VBO_ThreeD_1);
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices_ThreeD_1), vertices_ThreeD_1, GL_STATIC_DRAW);
    //(该属性位置(传入shader中的location), 该数据大小, 数据类型, 是否需要标准化, 总数据大小, 偏移量)
    //position attribute_location = 0
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);
