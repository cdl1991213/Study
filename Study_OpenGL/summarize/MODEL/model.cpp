...
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    for(unsigned int i = 0; i < mesh->mNumVertices; i++) {
        if(m_maxX<mesh->mVertices[i].x) m_maxX=mesh->mVertices[i].x;    //获取模型最大X坐标
        if(m_maxY<mesh->mVertices[i].y) m_maxY=mesh->mVertices[i].y;    //获取模型最大Y坐标
        if(m_minX>mesh->mVertices[i].x) m_minX=mesh->mVertices[i].x;    //获取模型最小X坐标
        if(m_minY>mesh->mVertices[i].y) m_minY=mesh->mVertices[i].y;    //获取模型最小Y坐标
        Vertex vertex;
        //处理顶点位置、法线和纹理坐标
        //顶点
        QVector3D vector;
    ...
    if(mesh->mMaterialIndex >= 0) {     //大于0表示AiMesh中存在纹理数据
        ...
        vector<Texture> emissionMaps =
                loadMaterialTextures(material, aiTextureType_EMISSION_COLOR, "texture_emission");
...
vector<Texture> Model::loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName)
{
    ...
    for(unsigned int i = 0; i < mat->GetTextureCount(type); i++) {
        aiString str;
        //(aiTextureType type, 第几个, 名称)
        mat->GetTexture(type, i, &str);
        bool skip = false;

        //textures_loaded中存放已加载的纹理图，遍历textures_loaded
        for(unsigned int j = 0; j < textures_loaded.size(); j++) {
            if(std::strcmp(textures_loaded[j].path.data(), str.C_Str()) == 0) {     //若当前目标纹理图判断为已加载，进入如下语句块。若当前目标纹理图判断为未加载，跳过循环
                textures.push_back(textures_loaded[j]);     //将已加载的同名纹理图代替存入textures
                skip = true;        //已加载，跳过后面加载纹理图的模块
                break;
            }
        }
        ...
        if(!skip){
            ...
            textures.push_back(texture);        //将纹理图存入textures
            textures_loaded.push_back(texture);     //已加载该纹理图，存入textures_loaded
        }
    }
    return textures;
}
...
unsigned int Model::TextureFromFile(const char *path, const string &directory)
{
    ...
    QOpenGLTexture * texture=new QOpenGLTexture(QImage(filename.c_str()).mirrored());   //绑定纹理

    //Debug
    if(texture==NULL) qDebug()<<"texture is NULL";
    else qDebug()<<filename.c_str()<<"loaded";

    return texture->textureId();    //返回纹理ID
}
