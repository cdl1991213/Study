...
void Mesh::Draw(QOpenGLShaderProgram &shader)   //读取纹理数据
{
    ...
    m_glFuns->glBindVertexArray(VAO);
//    m_glFuns->glDrawArrays(GL_TRIANGLES, 0, vertices.size());                 //无索引
    m_glFuns->glDrawElements(GL_TRIANGLES,indices.size(),GL_UNSIGNED_INT,0);    //有索引
}
